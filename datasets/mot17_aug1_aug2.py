...
class DetMOTDetection:
    ...
    def __getitem__(self, idx):
        ...
        if torch.rand(1).item() > self.data_ratio:
            images, targets = self.transform_for_shuffle(images, targets)
            is_augment = False
        else:
            images, targets = self.transform_for_pretrain(images, targets)
            is_augment = True
        ...
        # import pdb;pdb.set_trace()
        gt_instances = []
        # is_MOT17 = dataset_name == 'MOT17'
        dataset = []
        for img_i, targets_i in zip(images, targets):
            gt_instances_i = self._targets_to_instances(targets_i, img_i.shape[1:3])
            gt_instances.append(gt_instances_i)
            if is_augment:
                dataset.append(torch.tensor(1))
            else:
                dataset.append(torch.tensor(0))
        data.update({
            'imgs': images,
