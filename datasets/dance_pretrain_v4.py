...
class DetMOTDetection:
    ...
    def _targets_to_instances(targets: dict, img_shape) -> Instances:
        ...
        gt_instances.orig_size = targets['orig_size'].unsqueeze(0).repeat(targets['area'].shape[0], 1)
        return gt_instances
    ...
    def transform_box(self, box, img_w, img_h,
                    max_shift=30,
                    scale_range=(0.5, 1.5)):
        x1, y1, x2, y2 = box
        w, h = x2 - x1, y2 - y1
        ...
        cx = (x1 + x2) // 2 + random.randint(-max_shift, max_shift)
        cy = (y1 + y2) // 2 + random.randint(-max_shift, max_shift)

        nx1 = max(0, cx - nw // 2)
        ny1 = max(0, cy - nh // 2)
        nx2 = min(img_w, nx1 + nw)
        ny2 = min(img_h, ny1 + nh)
    ...
    def extract_patches(self, image, boxes):
        """
        image: Tensor[C,H,W]
    ...
    def paste_patch(self, image, patch, box):
        x1, y1, x2, y2 = map(int, box.tolist())
        patch = patch.resize((x2 - x1, y2 - y1), resample=Image.BILINEAR)
        image.paste(patch, (x1, y1))
        return image


    def generate_pseudo_video_and_targets(self, images, targets):
        """
        images  : List[Tensor[C,H,W]], len=n
        targets : List[Dict], len=1
        """
        ...
        boxes0 = tgt0['boxes']
        labels = tgt0['labels']
        obj_ids = tgt0['obj_ids']
        iscrowd = tgt0['iscrowd']
        dataset = tgt0.get('dataset', 'MOT17')

        # 1. patches from first frame
        patches = self.extract_patches(first_img, boxes0)

        new_images = []
        new_targets = []

        # ===== frame 0 =====
        new_images.append(first_img.copy())
        new_targets.append({k: v.clone() if torch.is_tensor(v) else v
                            for k, v in tgt0.items()})

        # ===== following frames =====
        active_indices = torch.arange(len(patches))
        prev_boxes = boxes0.clone()
        ...
        for t in range(1, n):
            ...
            cur_boxes = []
            cur_indices = [] 
            # order = torch.randperm(len(patches))  # occlusion randomness

            for k, idx in enumerate(active_indices):
                patch = patches[idx]
                prev_box = prev_boxes[k]

                new_box = self.transform_box(prev_box.tolist(), W, H)
                if new_box is None:
                    continue

                img = self.paste_patch(img, patch, new_box)
                cur_boxes.append(new_box)
                new_active_indices.append(idx)

            cur_boxes = torch.stack(cur_boxes, dim=0)
            active_indices = torch.as_tensor(new_active_indices, dtype=torch.long)
            keep_idx = active_indices

            area = (cur_boxes[:, 2] - cur_boxes[:, 0]) * \
                (cur_boxes[:, 3] - cur_boxes[:, 1])
            ...
            new_targets.append({
                'dataset': dataset,
                'boxes': cur_boxes,
                'labels': labels[keep_idx].clone(),
                'obj_ids': obj_ids[keep_idx].clone(),
                'iscrowd': iscrowd[keep_idx].clone(),
                'area': area,
                'image_id': tgt0['image_id'].clone() + t,
    ...
    def __getitem__(self, idx):
        ...
        targets = targets[0] #只保留第一帧的gt
        images, targets = self.generate_pseudo_video_and_targets(images, targets)
