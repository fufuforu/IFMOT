...
class ClipMatcher(SetCriterion):
    ...
    ,unmatched_track_ious=None):
        ...
        weight_mask = torch.ones_like(target_classes, dtype=torch.float)

        if unmatched_track_idxes is not None:
            weight_mask[0][unmatched_track_idxes] = unmatched_track_ious
        ...
        if self.focal_loss:
            ...
            loss_ce = sigmoid_focal_loss(src_logits.flatten(1),
                                             ...
                                             alpha=0.25,
                                             gamma=2,
                                             num_boxes=num_boxes, mean_in_dim1=False,return_per_query_loss=True)
            loss_ce = loss_ce.view(target_classes.shape)
            loss_ce = (loss_ce * weight_mask).sum()
        else:
            loss_ce = F.cross_entropy(src_logits.transpose(1, 2), target_classes, self.empty_weight)
            loss_ce = (loss_ce * weight_mask).sum() / weight_mask.sum()
    ...
    def match_for_single_frame_not_use_id(self, outputs: dict, box_features, last_box_features):
        ...
        if unmatched_track_idxes is not None:
            # import pdb;pdb.set_trace()
            unmatched_track_ious = track_instances.iou[unmatched_track_idxes]
        for loss in self.losses:
            new_track_loss = self.get_loss(loss,
                                           outputs=outputs_i,
                                           gt_instances=[gt_instances_i],
                                           ...
                                           unmatched_track_ious=unmatched_track_ious)
            self.losses_dict.update(
                {'frame_{}_{}'.format(self._current_frame_idx, key): value for key, value in new_track_loss.items()})
...
def build(args):
    ...
    if args.memory_bank_type is not None and len(args.memory_bank_type) > 0:
        ...
        memory_bank = None
    losses = ['labels', 'boxes']
    criterion = ClipMatcher(num_classes, matcher=img_matcher, weight_dict=weight_dict, losses=losses,soft_match = args.soft_match)
    criterion.to(device)
    postprocessors = {}
    model = MOTR(
        backbone,
...
#v9 加入soft match 对于unmatched track query加权计算,动态阈值 根据iou