...
class ClipMatcher(SetCriterion):
    ...
    def get_consistency_loss(self, first_output_embedding, end_output_embedding, device):
        if len(first_output_embedding) == 0:
            loss_emb = torch.zeros((), device=device)
        else:
            emb_first = torch.stack(first_output_embedding, dim=0)  
            emb_end   = torch.stack(end_output_embedding, dim=0)   
            emb_first = F.normalize(emb_first, dim=-1)
            emb_end   = F.normalize(emb_end, dim=-1)
            loss_emb = F.mse_loss(
                emb_first,
                emb_end.detach()
            )
            # import pdb;pdb.set_trace()
        consistency_loss = {'loss_consistency': loss_emb}
        return consistency_loss
        # import pdb;pdb.set_trace()

    def match_for_single_frame_without_loss(self, outputs: dict):
    ...
    def match_for_single_frame(self, outputs: dict):
        ...
        if self.consistency_loss: 
            if self.output_embedding is None:#记录第一帧的output embedding
                self.output_embedding = {}
                query_inds = matched_indices[:, 0]
                gt_inds = matched_indices[:, 1]

                for q, g in zip(query_inds.tolist(), gt_inds.tolist()):
                    self.output_embedding[g] = track_instances.output_embedding[q]
            else:
                first_output_embedding = []
                end_output_embedding = []
                for i in range(prev_matched_indices.shape[0]):
                    query_inds = prev_matched_indices[:, 0][i]
                    gt_inds = prev_matched_indices[:, 1][i]
                    # import pdb;pdb.set_trace()
                    
                    first_output_embedding.append(self.output_embedding[int(gt_inds)])
                    end_output_embedding.append(track_instances.output_embedding[int(query_inds)])
                    
                consistency_loss = self.get_consistency_loss(first_output_embedding, end_output_embedding, pred_logits_i.device)
                self.losses_dict.update(
                        {'frame_{}_{}'.format(self._current_frame_idx, key): value for key, value in consistency_loss.items()})
                    # raise RuntimeError(
                    #     # f"[Track–Patch mismatch]\n"
                    #     f"prev_matched_indices = {prev_matched_indices}\n"
                    #     f"prev_matched_indices.shape = {prev_matched_indices.shape}\n"
...
class MOTR(nn.Module):
    ...
    def _post_process_single_image(self, frame_res, track_instances, is_last, calculate_loss):
        with torch.no_grad():
            if self.training:
        ...
        if self.training:
            ...
            if calculate_loss:
                ...
                track_instances = self.criterion.match_for_single_frame_without_loss(frame_res)
            # import pdb;pdb.set_trace()
        else:
    ...
    def inference_single_image(self, img, ori_img_size, track_instances=None):
        ...
        # import pdb;pdb.set_trace()
        res = self._post_process_single_image(res, track_instances, False, False)
    ...
    def forward(self, data: dict):
        ...
        for frame_index, frame in enumerate(frames):
            ...
            calculate_loss = is_last or is_first
            ...
            frame_res = self._post_process_single_image(frame_res, track_instances, is_last, calculate_loss)
...
def build(args):
    ...
    num_frames_per_batch = max(args.sampler_lengths) * 2 - 1
    weight_dict = {}
    for i in range(num_frames_per_batch):
        weight_dict.update({"frame_{}_loss_ce".format(i): args.cls_loss_coef,
                            'frame_{}_loss_bbox'.format(i): args.bbox_loss_coef,
                            'frame_{}_loss_giou'.format(i): args.giou_loss_coef,
                            'frame_{}_loss_consistency'.format(i): args.consistency_loss_coef,
                            })
    ...
    if args.memory_bank_type is not None and len(args.memory_bank_type) > 0:
        ...
        memory_bank = None
    losses = ['labels', 'boxes']
    criterion = ClipMatcher(num_classes, matcher=img_matcher, weight_dict=weight_dict, losses=losses, consistency_loss=args.consistency_loss)
    criterion.to(device)
    postprocessors = {}
    model = MOTR(
        backbone,
