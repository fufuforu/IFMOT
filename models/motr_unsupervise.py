...
class MOTR(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, criterion, track_embed,
                 aux_loss=True, with_box_refine=False, two_stage=False, memory_bank=None, use_checkpoint=False, is_teacher=False):
    ...
    def _post_process_single_image(self, frame_res, track_instances, choice_for_loss, is_last):
        ...
        if self.training and (not self.is_teacher):
            ...
            # print("Teacher mode:", self.training)
            # print("Max score:", track_instances.scores.max().item())
            ...
            frame_res['pseudo_labels'] = track_instances[track_instances.obj_idxes>=0]
            # print(frame_res['pseudo_labels'])
    ...
    def make_pseudo_labels_to_gt(self, pseudo_gt_list, device):
        
        gt_instances_list = []
        for pseudo_gt in pseudo_gt_list:
            pseudo_gt = pseudo_gt[pseudo_gt.scores > self.track_base.score_thresh]
            gt_instances = Instances((1, 1))
            gt_instances.boxes = pseudo_gt.pred_boxes
            # import pdb;pdb.set_trace()
            gt_instances.labels = torch.zeros((len(pseudo_gt.pred_boxes), ), dtype=torch.long)
            gt_instances.obj_ids = pseudo_gt.obj_idxes
            gt_instances= gt_instances.to(device)
    ...
    def forward(self, data: dict):
        ...
        # 可视化 end
        if self.training and (not self.is_teacher):
            data['gt_instances'] = self.make_pseudo_labels_to_gt(data['pseudo_gt'],device=data['imgs'][0].device)
        ...
        outputs = {
            'pred_logits': [],
            'pred_boxes': [],
            'pseudo_labels': [], #only for teacher
        }
        ...
        for frame_index, (frame, gt) in enumerate(zip(frames, data['gt_instances'])):
            ...
            if self.is_teacher:
                outputs['pseudo_labels'].append(frame_res['pseudo_labels'])
        # 可视化 begin
        # from .visualizer import ImageVisualizer
        # visualizer = ImageVisualizer(denormalized=True)
