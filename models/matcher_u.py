    ...
    def forward(self, outputs, targets, use_focal=True):
        ...
        with torch.no_grad():
            ...
            if  C.numel() == 0:
                ...
                C = C.new_zeros(bs, num_queries, C.shape[-1]).cpu()
            else:
            ...
            if  C.numel() == 0:
                min_index = C.new_zeros(0, dtype=torch.int64)
            else:
                min_index = C[0].min(dim=0)[1][indices[0][1]]
            # import pdb;pdb.set_trace()
            return [(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j, dtype=torch.int64)) for i, j in indices], min_index
