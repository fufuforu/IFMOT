...
class ClipMatcher(SetCriterion):
    ...
    neglect_weight_idxes=None,
    neglect_loss_weights=None,):
    ...
    def loss_labels(
        ...
        # ------------------------------------------------------------
        # Query-level loss weights.
        # Default: all queries have weight 1.
        # For linear decay: unmatched track queries are assigned soft weights.
        # ------------------------------------------------------------
        ...
        if (
            neglect_weight_idxes is not None
            and neglect_loss_weights is not None
            and len(neglect_weight_idxes) > 0
        ):
            query_loss_weights[0, neglect_weight_idxes] = neglect_loss_weights.to(
                device=device,
                dtype=src_logits.dtype
            )
        ...
        # ------------------------------------------------------------
        # Hard ignore version:
        # directly remove ignored queries, same as your original implementation.
        # This branch is used for the original hard step-function.
        # ------------------------------------------------------------
        if unmatched_track_idxes is not None and len(unmatched_track_idxes) > 0:
            ...
            query_loss_weights = query_loss_weights[~ignore_mask].unsqueeze(dim=0)
        ...
        if self.focal_loss:
            ...
            # Expand query-level weights to class-level weights.
            # Shape:
            #   query_loss_weights: [B, N]
            #   class_loss_weights: [B, N, C]
            class_loss_weights = query_loss_weights.unsqueeze(-1).expand_as(
                gt_labels_target
            )

            loss_ce = loss_ce * class_loss_weights.flatten(1)
            loss_ce = loss_ce.sum()
            ...
            loss_ce = loss_ce * query_loss_weights
    ...
    def match_for_single_frame_not_use_id(self, outputs: dict, box_features, last_box_features):
        ...
        # ablation setting:
        # "hard": original hard step-function
        # "linear_decay": soft linear time-decaying weighting
        neglect_mode = 'linear_decay'

        if len(unmatched_track_idxes_final) > 0:
            disappear_time = track_instances.disappear_time[unmatched_track_idxes_final].float()

            if neglect_mode == "hard":
                # Original hard step-function:
                # tracks with disappear_time <= miss_tolerance are fully ignored.
                mask_ignore = disappear_time <= miss_tolerance
                true_ignore_idxes = unmatched_track_idxes_final[mask_ignore]

                # No soft weight is used in the original hard setting.
                neglect_weight_idxes = None
                neglect_loss_weights = None

                num_disappear_track = len(unmatched_track_idxes_final) - len(true_ignore_idxes)

            elif neglect_mode == "linear_decay":
                # Soft linear decay:
                # disappear_time = 1 -> loss weight = 0.0
                # disappear_time = 2 -> loss weight = 1 / miss_tolerance
                # disappear_time = 3 -> loss weight = 2 / miss_tolerance
                # disappear_time >= 4 -> loss weight = 1.0
                #
                # This avoids the abrupt transition between disappear_time = 3 and 4.
                neglect_loss_weights = torch.clamp(
                    (disappear_time - 1.0) / float(miss_tolerance),
                    min=0.0,
                    max=1.0
                )

                # In soft mode, we do not completely ignore these unmatched tracks.
                # Instead, we assign a soft loss weight to each unmatched track.
                true_ignore_idxes = torch.empty((0,), dtype=torch.long, device=device)

                # These indices and weights will be passed to loss_labels.
                neglect_weight_idxes = unmatched_track_idxes_final

                # Effective number of disappeared samples for loss normalization.
                num_disappear_track = float(neglect_loss_weights.sum().detach().item())

            else:
                raise ValueError(f"Unknown neglect_mode: {neglect_mode}")
        ...
        # --------------------------------------------------------------------------------
        # 状态感知特征更新 (State-Aware Feature Update, SAFU)
        # --------------------------------------------------------------------------------
        if len(matched_indices) > 0:
        ...
        for loss in self.losses:
            new_track_loss = self.get_loss(loss,
                                           ...
                                           neglect_weight_idxes=neglect_weight_idxes,
                                           neglect_loss_weights=neglect_loss_weights,) 
            self.losses_dict.update(
                {'frame_{}_{}'.format(self._current_frame_idx, key): value for key, value in new_track_loss.items()})
...
#joint版本，用id做监督
# 2026 6 10 为了消融实验加的版本，更改了公式13对应的部分，改为线性衰减